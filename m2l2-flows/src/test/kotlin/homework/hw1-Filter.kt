...
class Exercise1Filter {
    ...
    fun filter() = runBlocking {
        ...
        val res = LIST
            ...
            .run { flt.visibilitiesOr?.let { v -> this.filter { it.visibility in v } } ?: this }
            .run { flt.priceMin?.let { p -> this.filter { it.price >= p } } ?: this }
//            .run { flt.priceMax?.let { p -> this.filter { it.price <= p } } ?: this }
